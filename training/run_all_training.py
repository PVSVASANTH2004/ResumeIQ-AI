"""
ResumeIQ AI — Training Script
Trains all 3 models and saves them to data/models/
Run from the training/ directory: python run_all_training.py
"""
...
print("=" * 60)
print("  ResumeIQ AI — Full Training Pipeline")
print("=" * 60)
...
# ══════════════════════════════════════════════════════════════
print("\n[1/3] Resume Category Classifier (Kaggle Dataset)")
print("-" * 50)

# ── Load dataset ──────────────────────────────────────────────
POSSIBLE = [
    ('data/Resume.csv',               'Resume_str'),
    ('data/UpdatedResumeDataSet.csv', 'Resume'),
    ('data/resume.csv',               'Resume_str'),
]

df = None
TEXT_COL = None
for path, col in POSSIBLE:
    if os.path.exists(path):
        df = pd.read_csv(path)
        TEXT_COL = col if col in df.columns else (
            'Resume_str' if 'Resume_str' in df.columns else 'Resume'
        )
...
if df is None:
    print("  ERROR: Resume.csv not found in training/data/")
    sys.exit(1)
...
df = df[df['clean'].str.len() > 50].reset_index(drop=True)
...
models1 = {
    ...
    'Linear SVC':          CalibratedClassifierCV(LinearSVC(max_iter=2000, C=1.0)),
...
for name, model in models1.items():
    ...
    cvs = cross_val_score(model, X_tr, y_tr, cv=cv5, scoring='f1_macro', n_jobs=-1)
    res1[name] = {'model': model, 'acc': acc, 'f1': f1,
                  'cv': cvs.mean(), 'cv_std': cvs.std(), 'pred': y_pred}
    print(f"    {name:22s} | Acc: {acc:.4f} | F1: {f1:.4f} | CV F1: {cvs.mean():.4f} ± {cvs.std():.4f}")

best1_name = max(res1, key=lambda k: res1[k]['cv'])
best1 = res1[best1_name]
print(f"\n  Best model: {best1_name}")
print(f"\n{classification_report(y_te, best1['pred'], target_names=le.classes_, digits=4)}")

# ── Plot confusion matrix ─────────────────────────────────────
cm = confusion_matrix(y_te, best1['pred'])
fig, ax = plt.subplots(figsize=(16, 13))
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            xticklabels=le.classes_, yticklabels=le.classes_, ax=ax)
ax.set_title(f'Resume Classifier — Confusion Matrix ({best1_name})', fontweight='bold')
...
cdf = pd.DataFrame([
    {'Model': n, 'Accuracy': v['acc'], 'F1 Macro': v['f1'], 'CV F1': v['cv']}
    for n, v in res1.items()
]).set_index('Model')
ax = cdf.plot(kind='bar', figsize=(10,5), rot=20, colormap='Set2')
ax.set_title('Model Comparison — Resume Classifier', fontweight='bold')
ax.set_ylim(0, 1.1)
...
with open('../data/models/resume_classifier.pkl', 'wb') as f:
    pickle.dump({'model': best1['model'], 'tfidf': tfidf, 'label_encoder': le,
                 'model_name': best1_name, 'accuracy': best1['acc'],
                 'f1_macro': best1['f1'], 'cv_mean': best1['cv'],
                 'classes': list(le.classes_)}, f)
print(f"  Model saved -> data/models/resume_classifier.pkl")
print(f"  RESULT: Accuracy={best1['acc']*100:.2f}% | F1={best1['f1']:.4f} | CV={best1['cv']:.4f}")
...
def noise(s): return np.random.normal(0, s, N)

X2 = np.column_stack([
    clip(quality*0.80 + noise(0.15)),  # semantic_match
    clip(quality*0.75 + noise(0.15)),  # evidence_score
    clip(quality*0.70 + noise(0.18)),  # experience_quality
    clip(quality*0.65 + noise(0.20)),  # project_complexity
    clip(quality*0.72 + noise(0.15)),  # skill_confidence
    clip((1-quality)*0.40 + noise(0.10)),  # hallucination_rate
    clip((1-quality)*0.30 + noise(0.10)),  # keyword_stuffing_risk
    clip(quality*0.60 + noise(0.20)),  # skill_breadth
    clip(quality*0.50 + noise(0.25)),  # experience_count
    clip(quality*0.55 + noise(0.20)),  # project_count
    clip((1-quality)*0.50 + noise(0.15)),  # missing_required
    clip(quality*0.60 + noise(0.20)),  # total_tenure
])
...
with open('../data/models/hiring_recommender.pkl', 'wb') as f:
    pickle.dump({'model': model2, 'feature_names': FEATURE_NAMES,
                 'label_names': LABEL_NAMES2, 'accuracy': float(acc2),
                 'f1_weighted': float(f1w2), 'f1_macro': float(f1m2),
                 'roc_auc': float(roc2)}, f)
print(f"  Model saved -> data/models/hiring_recommender.pkl")
...
print(f"  Dataset: {len(df3)} samples (60 seed × ~8 augmented)")
...
with open('../data/models/project_complexity.pkl', 'wb') as f:
    pickle.dump({'clf': rf3, 'tfidf': tfidf3, 'label_names': LABEL_NAMES3,
                 'accuracy': float(acc3), 'f1_macro': float(f1m3),
                 'kappa': float(kap3), 'cv_mean': float(cv3.mean())}, f)
...
print("=" * 60)
print(f"\n  Model 1 — Resume Classifier   : Acc={best1['acc']*100:.2f}%  F1={best1['f1']:.4f}  ({best1_name})")
...
print(f"  Saved charts : training/plots/")
print("=" * 60)
